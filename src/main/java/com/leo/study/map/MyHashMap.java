    ...
    final Node<K, V>[] resize() {
        ...
        if (oldCap > 0) {
            ...
            if (oldCap >= MAXIMUM_CAPACITY) {
                // 如果旧table长度极大，则不扩容，将扩容门槛设为极大，直接返回旧table
                threshold = Integer.MAX_VALUE;
                return oldTab;
            } else if ((newCap = oldCap * 2) < MAXIMUM_CAPACITY &&
                    oldCap >= DEFAULT_INITIAL_CAPACITY) {
                // 新cap为旧cap翻倍
                // 若新cap比极大值小且旧cap大于等于默认值16，则新的门槛也翻倍
                newThr = oldThr * 2; // double threshold
            }
        } else if (oldThr > 0) { // initial capacity was placed in threshold
            // 如果旧cap=0且旧门槛大于0，则新cap等于旧门槛
            newCap = oldThr;
        } else {               // zero initial threshold signifies using defaults
            // 如果旧cap=0且旧门槛=0，则新cap=默认值16，新门槛=0.75*16=12
            newCap = DEFAULT_INITIAL_CAPACITY;
            newThr = (int) (DEFAULT_LOAD_FACTOR * DEFAULT_INITIAL_CAPACITY);
        }
        if (newThr == 0) {
            // 如果新门槛为0,判断0.75*newCap是否比极大值小，若是则新门槛为0.75*newCap,否则为最大值
