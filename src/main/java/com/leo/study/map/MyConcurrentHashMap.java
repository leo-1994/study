...
public class MyConcurrentHashMap<K, V> {
    ...
    // 表初始化和调整大小控制。 当为负时，表正在初始化或调整大小：-1表示初始化，否则 - （1 +活动大小调整线程数）。
    ...
    private final Node<K, V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            if ((sc = sizeCtl) < 0) {
                // 如果sizeCtl小于0，说明正处在初始化中
                // 线程让步，让当前线程由“运行状态”进入到“就绪状态”
    ...
    private final void transfer(Node<K, V>[] tab, Node<K, V>[] nextTab) {
        ...
        // 步幅
        int stride;
        if ((stride = (NCPU > 1) ? (n / 8) / NCPU : n) < MIN_TRANSFER_STRIDE) {
            // 如果cpu核数大于1，则stride=当前table长度/8再/cpu数，否则stride=cpu数
            // 如果strude<16，stride置为16
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K, V>[] tab = table; ; ) {
            ...
            if (tab == null || (n = tab.length) == 0) {
                tab = initTable();
            } else if ((f = tabAt(tab, i = hash % n)) == null) {   //根据hash值计算出在table里面的位置
                    ...
                    if (tabAt(tab, i) == f) {
                        ...
                        if (fh >= 0) {
                            ...
                            for (Node<K, V> e = f; ; ++binCount) {
                                ...
                                Node<K, V> pred = e;
                                //如果遍历到了最后一个结点，那么就证明新的节点需要插入 就把它插入在链表尾
                                if ((e = e.next) == null) {
    ...
    static {
        try {
            ...
            // numberOfLeadingZeros是求这个数据的二进制串中从最左边算起连续的“0”的总数量
